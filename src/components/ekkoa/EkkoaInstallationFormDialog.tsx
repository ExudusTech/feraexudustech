...
export default function EkkoaInstallationFormDialog({ open, onOpenChange, installation }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Block if coverage validation fails and areas exist
    if (hasAreas && !coverageResult.isValid) return;
    const payload = {
      ...
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button type="submit" disabled={isPending || (hasAreas && !coverageResult.isValid && !!(form.zip_code || form.city || form.state))}>
